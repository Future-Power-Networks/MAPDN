...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.Transition = namedtuple('Transition', ('state', 'action', 'log_prob_a', 'value', 'next_value', 'reward', 'next_state', 'done', 'last_step', 'action_avail'))
    ...
    def train_process(self, stat, trainer):
        ...
        for t in range(self.args.max_steps):
            ...
            action, action_pol, log_prob_a, _ = self.get_actions(state_, status='train', exploration=True, actions_avail=torch.tensor(trainer.env.get_avail_actions()), target=False)
            # print (f"This is the actions: {action_pol.size()}")
            ...
            next_state_ = cuda_wrapper(prep_obs(state).contiguous().view(1, self.n_, self.obs_dim), self.cuda_)
            _, next_action_pol, _, _ = self.get_actions(next_state_, status='train', exploration=True, actions_avail=torch.tensor(trainer.env.get_avail_actions()), target=False)
            # print (f"This is the next_action shape: {next_action.size()}")
    ...
    def evaluation(self, stat, trainer):
        ...
        for episode in range(eval_times):
            ...
            for t in range(self.args.max_steps):
                state_ = cuda_wrapper(prep_obs(state).contiguous().view(1, self.n_, self.obs_dim), self.cuda_)
                action, _, _, _ = self.get_actions(state_, status='test', exploration=False, actions_avail=torch.tensor(trainer.env.get_avail_actions()), target=False)
                _, actual = translate_action(self.args, action, trainer.env)
                reward, done, info = trainer.env.step(actual)
                done_ = done or t==self.args.max_steps-1
                next_state = trainer.env.get_obs()
    ...
    def unpack_data(self, batch):
        ...
        if self.args.reward_normalisation:
            reward = batchnorm(reward)
        return (state, action, log_prob_a, value, next_value, reward, next_state, done, last_step, action_avail)
